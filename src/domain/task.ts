...
export type CircuitCondition =
  ...
  | {
      /**
       * Ослабление на заданной частоте (М4): ЭДС источника ~ не менее чем
       * в `atLeast` раз больше амплитуды (пика) напряжения на Компоненте-
       * выходе. Измеряется фазорным свипом — схема решается на частоте
       * условия, а не на частоте источника.
       */
      readonly kind: 'attenuates-frequency';
      /** Компонент-выход фильтра (например, конденсатор ФНЧ или резистор ФВЧ). */
      readonly componentKind: ComponentKind;
      /** Частота измерения ослабления, Гц. */
      readonly frequency: number;
      /** Минимальное ослабление, раз; граница включительна. */
      readonly atLeast: number;
    }
  | {
      /**
       * Форма выпрямленного сигнала (М4): на Компоненте-нагрузке — пульсации
       * одной полярности (от нуля вверх) с пиком в границах. Измеряется в
       * переходном режиме по установившейся половине плана.
       */
      readonly kind: 'rectified-output';
      /** Компонент-нагрузка выпрямителя. */
      readonly componentKind: ComponentKind;
      /** Границы пика выходных пульсаций, В. */
      readonly range: MeasurementRange;
    };
